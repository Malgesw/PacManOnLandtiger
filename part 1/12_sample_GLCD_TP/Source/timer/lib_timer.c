...
void reset_timer( uint8_t timer_num )
{
  uint32_t regVal;
	...
	// first bit = 0/1 => disable/enable timer, second bit = 1 => reset timer
...
uint32_t init_timer ( uint8_t timer_num, uint32_t Prescaler, uint8_t MatchReg, uint8_t SRImatchReg, uint32_t TimerInterval )
{
  if ( timer_num == 0 )
  {
		LPC_TIM0-> PR = Prescaler; // Prescale Register (PR): when the Prescale Counter (PC) reaches this value,
		// the TC (Timer/Counter) is incremented (actually, every PR+1 clock cycles TC is incremented)
		
		if (MatchReg == 0){
			LPC_TIM0->MR0 = TimerInterval;
			LPC_TIM0->MCR |= SRImatchReg << 3*MatchReg; // Match Control Register (MCR): 12 available bits, 
			// 3 control bits for each match register
		}
		else if (MatchReg == 1){
