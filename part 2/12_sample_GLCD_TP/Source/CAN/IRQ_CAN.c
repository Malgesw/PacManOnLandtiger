...
extern uint16_t lifeLost;
extern uint16_t isDrawingLife;
...
void CAN_IRQHandler (void)  {
  ...
  if (icr & (1 << 0)) {                          		/* CAN Controller #1 meassage is received */
    ...
    LPC_CAN2->CMR = (1 << 2);                    		/* Release receive buffer */
		...
		uint8_t tC = CAN_RxMsg.data[0]; // timerCount
		uint8_t lC = CAN_RxMsg.data[1]; // lifeCount
		uint16_t cC = (CAN_RxMsg.data[2] << 8) | CAN_RxMsg.data[3]; // coinCount
		...
		if(lifeLost && isDrawingLife){
			GUI_Text((2+2*(lC))*cell_width, 30.5*cell_height, (uint8_t *)str, Black, Black);
			isDrawingLife = 0;
		}
		else if(isDrawingLife){
			drawPacMan((2 + 2*(lC))*cell_width, 30.5*cell_height, cell_width, cell_height, Yellow);
			isDrawingLife = 0;
		}
	}
